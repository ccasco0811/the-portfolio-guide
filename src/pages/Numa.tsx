import { useScrollAnimation, fadeInUp, fadeIn, scaleIn, staggeredFadeInUp } from '../hooks/useScrollAnimation'
...
export default function Numa() {
  ...
  const projectImageAnimation = useScrollAnimation({ threshold: 0.1 })
  const contextAnimation = useScrollAnimation()
  const insightsAnimation = useScrollAnimation()
  const roleAnimation = useScrollAnimation()
  const approachAnimation = useScrollAnimation()
  const journeyAnimation = useScrollAnimation()
  const solutionAnimation = useScrollAnimation()
  const flowAnimation = useScrollAnimation()
  const impactAnimation = useScrollAnimation()
  const ctaAnimation = useScrollAnimation()
  const useCase1Animation = useScrollAnimation()
  const useCase2Animation = useScrollAnimation()
  const useCase3Animation = useScrollAnimation()
    ...
    <div className="min-h-screen bg-white font-epilogue">
      ...
      <nav className="relative">
        <div className="flex items-center justify-between px-4 sm:px-10 py-4 max-w-7xl mx-auto">
          ...
          <div className="hidden md:flex items-center gap-10">
            ...
            <button className="px-4 py-4 border border-black rounded-full bg-white hover:bg-gray-50 hover:scale-105 transition-all duration-300 ease-in-out">
        ...
        <div className={`md:hidden fixed inset-0 bg-black bg-opacity-50 z-40 transition-opacity duration-300 ${isMobileMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
          <div className={`fixed top-0 right-0 w-64 h-full bg-white shadow-xl transform transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
            <div className="flex flex-col p-6 pt-20">
              <div className="flex flex-col gap-6">
                <a
                  href="#projects"
                  className="text-black text-lg font-normal py-3 hover:text-gray-600 transition-colors"
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  Projects
                </a>
                <button
                  className="px-6 py-4 border border-black rounded-full bg-white hover:bg-gray-50 transition-colors text-left"
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  <span className="text-black text-base font-normal">Let's connect</span>
                </button>
      ...
      <div className="flex flex-col items-center gap-20 px-10 pb-20 max-w-7xl mx-auto">
        ...
        <section
          ...
          className="flex flex-col max-w-2xl w-full gap-8"
          style={fadeInUp(heroAnimation.isVisible)}
        >
          <h1 className="text-lg font-medium text-black">Numa</h1>
          <h2 className="text-5xl sm:text-5xl text-[41px] font-bold text-black leading-tight">
        ...
        {/* Main Project Image */}
        <section
          ref={projectImageAnimation.elementRef}
          className="w-full max-w-6xl"
          style={scaleIn(projectImageAnimation.isVisible)}
        >
          <img 
        ...
        <div className="flex flex-col max-w-2xl w-full gap-20">
          ...
          {/* Context Section */}
          <section
            ref={contextAnimation.elementRef}
            className="flex flex-col gap-10"
            style={fadeInUp(contextAnimation.isVisible)}
          >
          ...
          <section
            ...
            className="flex flex-col gap-10"
            style={fadeInUp(insightsAnimation.isVisible)}
          >
          ...
          {/* My Role Section */}
          <section
            ref={roleAnimation.elementRef}
            className="flex flex-col gap-10"
            style={fadeInUp(roleAnimation.isVisible)}
          >
          ...
          {/* Approach Section */}
          <section
            ref={approachAnimation.elementRef}
            className="flex flex-col gap-5"
            style={fadeInUp(approachAnimation.isVisible)}
          >
            <h3 className="text-2xl font-bold text-gray-900">Approach</h3>
            <div className="flex flex-col gap-5">
              <div className="bg-[#2D4BAE] rounded-lg p-5 hover:bg-[#3a5bc7] hover:scale-[1.02] transition-all duration-300 ease-in-out cursor-pointer">
        ...
        <section
          ref={journeyAnimation.elementRef}
          className="w-full max-w-[1016px] inline-flex flex-col justify-start items-center gap-20"
          style={fadeInUp(journeyAnimation.isVisible)}
        >
          ...
          <div className="self-stretch inline-flex justify-center items-center gap-5 flex-wrap content-center">
            {[
              {
                title: "Awareness",
                description: "Discovers NUMA through Google searches, Facebook ads, and word of mouth. Learns there is a simpler way to invoice using WhatsApp.",
                gap: "gap-4",
                icon: "https://cdn.builder.io/api/v1/image/assets%2F3945ea7ebea44eb08f3599534ac73d69%2Fd7d5e8b6d0834ac5871070cc6cfda21a"
              },
              {
                title: "Consideration",
                description: "Visits landing page, watches a 30 second video, calculates savings, and shares phone number and email.",
                gap: "gap-4",
                icon: "https://cdn.builder.io/api/v1/image/assets%2F3945ea7ebea44eb08f3599534ac73d69%2Fc7fb22a321044ee692c945290732aca9"
              },
              {
                title: "Decision",
                description: "Requests more details through WhatsApp, asks colleagues, and signs up for early access.",
                gap: "gap-4",
                icon: "https://cdn.builder.io/api/v1/image/assets%2F3945ea7ebea44eb08f3599534ac73d69%2Ffbd708e178f64a1c806f75c296961ce9"
              },
              {
                title: "Onboarding",
                description: "Uploads a CSD (digital tax certificate required by Mexico's SAT), receives the shared WhatsApp number, issues a first invoice, and shares it with staff.",
                gap: "gap-2",
                icon: "https://cdn.builder.io/api/v1/image/assets%2F3945ea7ebea44eb08f3599534ac73d69%2F966220f08d71429eb47adb63c3e38330"
              },
              {
                title: "Adoption",
                description: "Starts invoicing daily, reviews summaries, and reduces reliance on an accountant.",
                gap: "gap-2",
                icon: "https://cdn.builder.io/api/v1/image/assets%2F3945ea7ebea44eb08f3599534ac73d69%2Fb3ea99ee896e4abfabf454328b94672c"
              },
              {
                title: "Advocacy",
                description: "Recommends NUMA in WhatsApp groups, shares experience on social media, and refers other business owners.",
                gap: "gap-2",
                icon: "https://cdn.builder.io/api/v1/image/assets%2F3945ea7ebea44eb08f3599534ac73d69%2Fb3ea99ee896e4abfabf454328b94672c"
              }
            ].map((item, index) => (
              <div key={index} className="max-w-80 max-h-80 flex justify-center items-center gap-5">
                <div
                  className="w-80 h-80 max-w-80 min-w-80 max-h-80 p-7 bg-white rounded-[40px] outline outline-2 outline-offset-[-2px] outline-black inline-flex flex-col justify-start items-start gap-4 hover:shadow-lg hover:scale-[1.02] transition-all duration-300 ease-in-out cursor-pointer"
                  style={staggeredFadeInUp(journeyAnimation.isVisible, index, 0.15)}
                >
                  <img className="w-24 h-24" src={item.icon} alt="Journey step icon" />
                  <div className={`self-stretch flex flex-col justify-start items-start ${item.gap}`}>
                    <div className="self-stretch justify-start text-black text-sm font-light font-['Epilogue'] leading-7">{item.title}</div>
                    <div className="self-stretch justify-start text-black text-base font-normal font-['Epilogue'] leading-normal">{item.description}</div>
                  </div>
                </div>
              </div>
            ))}
        ...
        <section
          ...
          className="flex flex-col gap-20 max-w-2xl w-full"
          style={fadeInUp(solutionAnimation.isVisible)}
        >
        ...
        <section
          ref={flowAnimation.elementRef}
          className="flex flex-col gap-20 max-w-2xl w-full"
          style={fadeInUp(flowAnimation.isVisible)}
        >
          <div className="flex flex-col gap-10">
        ...
        {/* Use Case 1: Preventing Blacklisted Client Transactions */}
        <section
          ref={useCase1Animation.elementRef}
          className="flex flex-col justify-center items-center gap-6 w-full max-w-6xl"
          style={fadeInUp(useCase1Animation.isVisible)}
        >
          <div className="flex items-center gap-2 w-full">
            <h4 className="flex-1 text-base font-light text-gray-900 leading-8">Preventing Blacklisted Client Transactions</h4>
          </div>
          <div className="flex flex-col lg:flex-row justify-center items-center gap-10 lg:gap-9 py-6 w-full flex-wrap">
            <img
              src="https://api.builder.io/api/v1/image/assets/TEMP/4128115560e117d460cff5e8bca43bf584afe2c6?width=580"
              alt="Use case mockup - Blacklisted transactions"
              className="w-72 h-auto aspect-[29/60]"
            />
            <p className="text-base text-gray-600 leading-relaxed flex-1 min-w-80 max-w-md">
              In Mexico, invoicing companies on the government blacklist can result in severe penalties and invalidate tax deductions. NUMA's real time validation against government databases protects users from unknowingly engaging in high risk transactions, similar to how KYC checks prevent money laundering in financial services.
            </p>
          </div>
        </section>

        {/* Use Case 2: Payment Method Intelligence */}
        <section
          ref={useCase2Animation.elementRef}
          className="flex flex-col justify-center items-center gap-6 w-full max-w-6xl"
          style={fadeInUp(useCase2Animation.isVisible)}
        >
          <div className="flex items-center gap-2 w-full">
            <h4 className="flex-1 text-base font-light text-gray-900 leading-8">Payment Method Intelligence</h4>
          </div>
          <div className="flex flex-col lg:flex-row justify-center items-center gap-10 lg:gap-9 py-6 w-full flex-wrap">
            <img
              src="https://api.builder.io/api/v1/image/assets/TEMP/2854a492173d1586c5ee6ea33a43eaf8a2a424e4?width=580"
              alt="Use case mockup - Payment intelligence"
              className="w-72 h-auto aspect-[29/60]"
            />
            <p className="text-base text-gray-600 leading-relaxed flex-1 min-w-80 max-w-md">
              NUMA doesn't just process invoices, it understands complex tax regime rules and limits. This AI-powered foresight prevents clients from accidentally violating their tax status, which could trigger audits and penalties. It's like having a tax expert reviewing every transaction in real-time.
            </p>
          </div>
        </section>

        {/* Use Case 3: Preventing Blacklisted Client Transactions */}
        <section
          ref={useCase3Animation.elementRef}
          className="flex flex-col justify-center items-center gap-6 w-full max-w-6xl"
          style={fadeInUp(useCase3Animation.isVisible)}
        >
          <div className="flex items-center gap-2 w-full">
            <h4 className="flex-1 text-base font-light text-gray-900 leading-8">Preventing Blacklisted Client Transactions</h4>
          </div>
          <div className="flex flex-col lg:flex-row justify-center items-center gap-10 lg:gap-9 py-6 w-full flex-wrap">
            <img
              src="https://api.builder.io/api/v1/image/assets/TEMP/160fa7b3e79b537286b93cb657f921ce4e1f1f53?width=580"
              alt="Use case mockup - Blacklisted transactions"
              className="w-72 h-auto aspect-[29/60]"
            />
            <p className="text-base text-gray-600 leading-relaxed flex-1 min-w-80 max-w-md">
        ...
        <section
          ...
          className="flex flex-col gap-20 w-full max-w-6xl justify-start items-center"
          style={fadeInUp(impactAnimation.isVisible)}
        >
          <div className="flex flex-col gap-6 max-w-2xl">
            <h3 className="text-2xl font-bold text-gray-900 mx-auto">Impact & Results</h3>
            <div className="flex flex-col gap-6">
              <p className="text-base text-gray-600 leading-relaxed">
        ...
        <section
          ...
          className="flex flex-col gap-10 max-w-2xl w-full"
          style={fadeInUp(ctaAnimation.isVisible)}
        >
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-gray-900 leading-tight">
            Interested in how I design products
          </h2>
          <button className="px-4 py-4 border border-black rounded-full bg-white hover:bg-gray-50 hover:scale-105 transition-all duration-300 ease-in-out self-start">
            <span className="text-black text-base font-normal">Let's connect</span>
          </button>
        </section>
      </div>
